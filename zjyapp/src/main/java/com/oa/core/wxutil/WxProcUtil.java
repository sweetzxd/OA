...
public class WxProcUtil {
    ...
    public ModelAndView WorkFlowProc(String userId,String formid,String wkflwId,String recno,String procid,String wkfNode,String workOrderNO) {
        ...
        try {
            ...
            for (int i = 0; i < nextNodeIds.size(); i++) {
                ...
                ts.setAccepter(userId);
                ts.setMsgStatus(4);
                taskSenderService.update(ts);
                if (path1ToOtherTask == 0) {
    ...
    public String getEmpUser(String user){
        ...
        if (user != null ) {
            ...
            String sql = "select d.deptHead from employees e join department d on e.department=d.deptId where e.curStatus=2 and d.curStatus=2 and username in ('"+user+"')";
    ...
    public String getDeptUser(String dept){
        ...
        if (dept != null ) {
            ...
            String sql = "select deptHead from  department where curStatus=2 and deptId in ('"+dept+"')";
            List<String> duser = tableService.selectSql(sql);
            actors = StringHelper.list2String(duser,";");
        }
        return actors;
