...
oA.success = OA.object("showContent", function (data) {
    ...
    this.convert = function () {
        for (var key in oA.field) {
            for (var val in oA.thead) {
    ...
    this.tbody = function () {
        var num = 0;
        var object = OA.object();

        for (var val in data.tbody) {
            object[num] = OA.array();
            object[num].push(num+1);
            for (var key in data.field) {
                if (Object.keys(data.tbody[val]).includes(data.field[key].name)) {
                    if(data.field[key].type=="form" || data.field[key].type=="forms" || data.field[key].type=="select"  || data.field[key].type=="selects"){
                        object[num].push(oa.decipher(data.field[key].type,data.field[key].name,data.tbody[val][data.field[key].name]));
                    }else{
