...
public class WxNoticeController {
    ...
    @ResponseBody
    public String seeFile(HttpServletRequest request,String id){
        JSONObject jsonObject = new JSONObject();
        try{
            String sql = "SELECT fileName,uploadTime,intro,fileAdd,remark,recordName FROM commonfiles WHERE curStatus=2 AND fileid='"+id+"'";
            Map<String,Object> map = tableService.selectSqlMap(sql);
            jsonObject.put("data",map);
            jsonObject.put("success",true);
        }catch (Exception e){
