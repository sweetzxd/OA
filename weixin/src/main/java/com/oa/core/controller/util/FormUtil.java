...
public class FormUtil {
    ...
    public List<HashMap> getFormHtml(String userId,TaskData taskData,String recorderNO,String pkid){
        ...
        if (recorderNO != null && !recorderNO.equals("newformdata")) {
            ...
            if(pkid!=null){
                sqlvalue = t.selectSqlMap(MySqlUtil.getFieldSql3(pkid,fields, tableId, where,"",""));
            }else {
        ...
        for (int i = 0, n = fields.size(); i < n; i++) {
            ...
            if (fieldType != null && fieldType.equals("child")) {
                ...
                if (FieldTypeUtil.isNull(value) && !FieldTypeUtil.isNull(special)) {
                    Hashtable ht = FieldTypeUtil.getSpecial(userId,special);
    ...
    /*public String getIframe(String tableId,FieldData fieldData,String value){
        String title = fieldData.getFieldTitle();
        String fieldHtml ="<div class='circumstance-bar box-bar'>";
        fieldHtml += "<div class='circumstance-title cell'>"+title+"</div></div>";
        *//*fieldHtml += "<div class='objection-right box-bar-list'>";
        fieldHtml += "<iframe id = ";*//*
        return fieldHtml;
    }*/
    ...
    public List<Map<String,Object>> getTableValue(String tableId ,Vector<String> fields,List<String> where, String limit,String pkid){
        String order = "order by recordTime desc";
        if(pkid!=null){
            fields.add(pkid+" as recorderNO");
            return t.selectSqlMapList(MySqlUtil.getFieldSql2(fields, tableId, where, order, limit));
    ...
    public List<Map<String,Object>> getTableValue2(String tableId ,Vector<String> fields,List<String> where, String page,String pkid){
        String order = "order by recordTime desc";
        String limit;
        int startRow = 0;
        if (page != null) {
        ...
        if(pkid!=null){
            pkid = pkid+" as recorderNO ";
            return t.selectSqlMapList(MySqlUtil.getFieldSql3(pkid,fields, tableId, where,order, limit));
        }else {
            return t.selectSqlMapList(MySqlUtil.getSql(fields, tableId, where, order, limit));
        }
    }
