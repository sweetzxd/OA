...
<!--startprint1-->
	<div style="width: 756px;height: 555px;">
		<table style="width: 756px;height: 70px;">
			<tr>
				<td style="width: 756px;font-size:25px;height:70px;top:20px;" align='center' valign='middle' colspan="4">
					河北省质检研究院工作人员年休假调整审批表
				</td>
			</tr>
		</table>
		<table border="1" cellspacing="0" style="width: 756px;height: 400px">
			<tr style="height: 50px;">
				<td style="width: 120px;text-align: center">部门</td>
			...
			<tr style="height: 50px;">
				<td style="width: 120px;text-align: center">原计划休假时间</td>
				<td style="width: 610px;text-align: center" colspan="5">
					<c:if test="${data.get(0).size()>0 && data.get(0) != null}">
						<c:forEach items="${data.get(0)}" var="da" varStatus="status">
							${da.get("yjhks")}至${da.get("yjhjs")}，共${da.get("yjhts")}天
						</c:forEach>
					</c:if>
				</td>
			</tr>
			<tr style="height: 50px;">
				<td style="width: 120px;text-align: center">调整后休假时间</td>
				<td style="width: 610px;text-align: center" colspan="5">
					<p style=" text-align: left;margin: 0 0 0 100px;"><label>已休 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;天,未休&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 天</label></p><br>
					<c:if test="${data.get(0).size()>0 && data.get(0) != null}">
						<c:forEach items="${data.get(0)}" var="da" varStatus="status">
							补休计划：${da.get("dzhks")}至${da.get("dzhjs")}，共${da.get("dzxjt")}天
						</c:forEach>
					</c:if>
				</td>
			</tr>
			<tr style="height: 50px;min-height:50px;" aria-rowspan="2">
				<td style="width: 120px;text-align: center">部门负责人意见</td>
				<td style="width: 610px;text-align: center" colspan="5">
					<c:if test="${data.get(0).size()>0 && data.get(0) != null}">
						<c:forEach items="${data.get(1)}" var="da" varStatus="status">
							<label>${da.get("yj")}</label>
							<label style="height: 22px;">${da.get("NAME")}</label>
							<label>${da.get("time")}</label>
						</c:forEach>
					</c:if>
				</td>
			</tr>
			<tr style="height:50px;min-height:50px;" aria-rowspan="2">
				<td style="width: 120px;text-align: center">人力资源部意见</td>
				<td style="width: 610px;text-align: center" colspan="5">
					<c:if test="${data.get(0).size()>0 && data.get(0) != null}">
			...
			<tr style="height:50px;min-height:50px;" aria-rowspan="2">
				<td style="width: 120px;text-align: center">主管院长意见</td>
				<td style="width: 610px;text-align: center" colspan="5">
					<c:if test="${data.get(0).size()>0 && data.get(0) != null}">
			...
			<tr style="height:50px;min-height:50px;" aria-rowspan="2">
				<td style="width: 120px;text-align: center">院长意见</td>
				<td style="width: 610px;text-align: center" colspan="5">
					<c:if test="${data.get(0).size()>0 && data.get(0) != null}">
