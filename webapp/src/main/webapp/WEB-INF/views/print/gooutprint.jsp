...
<c:forEach items="${datali}" var="data" varStatus="status" >
<!--startprint1-->
    <div style="width: 756px;margin: 0 auto;">
        <table style="width: 756px;height: 90px">
            <tr>
                <td style="width: 756px;font-size:30px;height:90px" align='center' valign='middle' colspan="4">出差审批单
                </td>
            </tr>
        </table>
        <table border="1" cellspacing="0" style="width: 756px;height: 450px; overflow-y:scroll">
            <tr style="height: 10%">
                <td style="width: 25%;text-align: center">出差部门</td>
                ...
                <td style="width: 25%; text-align: center">出差人</td>
                <td style="width: 25%; text-align: center">
                    <c:if test="${data.get(0).size()>0 && data.get(0) != null}">
                    <c:forEach items="${data.get(0)}" var="da" varStatus="status">
                        <c:set var="cctxr" value="${da.get('cctxr')}"/>
                        <c:set var="people" value="${da.get('people')}"/>
                        ${cctxr}<c:if test="${!fn:contains(cctxr,people)}">${people}</c:if>
                    </c:forEach>
                </c:if></td>
            </tr>
            <tr style="height: 8%">
                <td style="width: 20%;text-align: center">出差事由</td>
            ...
            <tr style="height: 12%">
                <td style="width: 25%;text-align: center">出差时间</td>
                <td style="width: 25%;text-align: center">
                    <c:if test="${data.get(0).size()>0 && data.get(0) != null}">
                    <c:forEach items="${data.get(0)}" var="da" varStatus="status">
                            <div>${fn:substring(da.get("begintime"),0,16)}</div>
                            <div>${fn:substring(da.get("endtime"),0,16)}</div>
                    </c:forEach>
                </c:if></td>
                <td style="width: 25%; text-align: center">出差方式</td>
                <td style="width: 25%; text-align: center">
                    <c:if test="${data.get(0).size()>0 && data.get(0) != null}">
                    <c:forEach items="${data.get(0)}" var="da" varStatus="status">
                        ${da.get("cctype")}
            ...
            <tr style="height: 8%">
                <td style="width: 20%;text-align: center">路线</td>
                <td style="width: 80%;text-align: left;" colspan="4">
                    <c:if test="${data.get(0).size()>0 && data.get(0) != null}">
                        <c:forEach items="${data.get(0)}" var="da" varStatus="status">
                                <label style="margin-left: 20px;">${da.get("address")}</label>
                               <%-- <label>${da.get("begintime")}</label>
                                <label>${da.get("endtime")}</label>--%>
                        </c:forEach>
                    </c:if>
                </td>
            </tr>
            <tr style="height: 10%">
                <td style="width: 20%;text-align: center">部门负责人审批</td>
                <td style="width: 80%;text-align: center" colspan="4">
                    <c:if test="${data.get(1).size()>0 && data.get(1) != null}">
                        <c:forEach items="${data.get(1)}" var="da" varStatus="status">
                            <div class="msg-box">
                                <label class="title">${da.get("bzyijian")}</label>
                                <label class="name" style="width: 120px;height: 50px">${da.get("bzname")}</label>
                                <label class="time">${da.get("bztime")}</label>
                            </div>
                        </c:forEach>
                    </c:if>
                </td>
            ...
            <tr style="height:10%">
                <td style="width: 20%;text-align: center">部门主管院长审批</td>
                <td style="width: 80%;text-align: center;" colspan="4">
                    <c:if test="${data.get(2).size()>0 && data.get(2) != null}">
                        <c:forEach items="${data.get(2)}" var="da" varStatus="status">
                            <div class="msg-box">
                                <label class="title">${da.get("zgyzyijian")}</label>
                                <label class="name" style="width: 120px;height: 50px">${da.get("zgyzname")}</label>
                                <label class="time">${da.get("zgyztime")}</label>
                            </div>
                        </c:forEach>
                    </c:if>
                </td>
            ...
            <tr style="height:15%">
                <td style="width: 20%;text-align: center">书记审批</td>
                <td style="width: 80%;text-align: center;" colspan="4">
                    <c:if test="${data.get(3).size()>0 && data.get(3) != null}">
                        <c:forEach items="${data.get(3)}" var="da" varStatus="status">
                            <div class="msg-box">
                                ...
                                <label class="name" style="width: 120px;height: 50px">${da.get("yzname")}</label>
