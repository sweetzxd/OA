...
<table style="width: 756px;height: 90px;padding-top: 35px">
    <tr>
        <td style="width: 756px;font-size:30px;height:90px" align='center' valign='middle' colspan="4">预付款结算申报单
        </td>
    </tr>
</table>
<table style="width: 756px;height: 20px; overflow-y:scroll">
    <tr>
        <td style="text-align: left" width="25%">申请人：${data.get(13).get("sqr")}</td>
        <td style="text-align: left" width="25%">部门：${data.get(0).get("bm")}</td>
        <td style="text-align: left" colspan="2" width="50%">申请时间：${data.get(10).get("year")}年${data.get(11).get("month")}月${data.get(12).get("day")}日</td>
    </tr>
</table>
<table border="1" cellspacing="0" style="width: 756px;height: 450px; overflow-y:scroll">
      <tr>
        <td style="text-align: center" width="20%">收款单位</td>
        <td style="text-align: center">${data.get(1).get("skdw200513002")}</td>
        <td width="20%" style="text-align: center">开户银行</td>
        <td style="text-align: center">${data.get(1).get("khx2005130002")}</td>
    </tr>
    <tr>
        <td style="text-align: center" width="20%">用途</td>
        <td style="text-align: center">${data.get(1).get("yt20051300002")}</td>
        <td style="text-align: center" width="20%">账号</td>
        <td style="text-align: center">${data.get(1).get("zh20051300002")}</td>
    </tr>
    <tr>
        <td style="text-align: center" width="20%">金额</td>
        <td style="text-align: center">￥${data.get(1).get("je20051300002")}</td>
        <td style="text-align: center" width="20%">支出类型</td>
        <td style="text-align: center">${data.get(14).get("zclx")}</td>
    </tr>
    <tr>
        <td style="text-align: center" width="20%">部门负责人</td>
        <td style="text-align: center" colspan="3">${data.get(8).get("bmfzr")}</td>
    </tr>
    <tr>
        <td style="text-align: center" width="20%">部门主管院长</td>
        <td style="text-align: center" colspan="3">${data.get(6).get("bmzg")}</td>
    </tr>
    <tr>
        <td style="text-align: center" width="20%">财务主管院长</td>
        <td style="text-align: center" colspan="3">${data.get(7).get("cwzg")}</td>
    </tr>
    <tr>
        <td style="text-align: center" width="20%">院长</td>
        <td style="text-align: center" colspan="3">${data.get(9).get("yz")}</td>
...
<table style="width: 756px;height: 20px; overflow-y:scroll">
    <tr>
        ...
        <td style="text-align: right" width="50%">2万元及以上需财务主管院长和院长签字</td>
    </tr>
</table>
<!--endprint1-->
</div>
