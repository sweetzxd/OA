...
<table style="width: 756px;height: 90px;padding-top: 35px">
    <tr>
        <td style="width: 756px;font-size:30px;height:90px" align='center' valign='middle' colspan="4">预借发票申请单
        </td>
    </tr>
</table>
...
<table border="1" cellspacing="0" style="width: 756px;height: 450px; overflow-y:scroll">
    <tr>
        ...
        <td style="text-align: center">${data.get(0).get("dfdwmX20101402")}</td>
        <td style="text-align: center" width="20%">纳税人识别号</td>
        <td style="text-align: center">${data.get(0).get("nsrsbX20101402")}</td>

    </tr>
      <tr>
          <td style="text-align: center" width="20%">金额(元)</td>
          <td style="text-align: center">${data.get(0).get("jeX20101400002")}</td>
        <td width="20%" style="text-align: center">金额大写</td>
        <td style="text-align: center">${data.get(0).get("jedxX201014002")}</td>
    </tr>
    <tr>
        <td style="text-align: center" width="20%">发票张数</td>
        <td style="text-align: center">${data.get(0).get("fpzsX201014002")}</td>
        <td style="text-align: center" width="20%">发票类型</td>
        <td style="text-align: center">${data.get(0).get("fplxX201014002")}</td>
    </tr>
    <tr>
        <td style="text-align: center" width="20%">核销日期</td>
        <td style="text-align: center">${data.get(0).get("hxrqX201014002")}</td>
        <td style="text-align: center" width="20%">开户行账号</td>
        <td style="text-align: center">${data.get(0).get("khxX2010140002")}${data.get(0).get("yxzhX201014002")}</td>
    </tr>
    <tr>
        <td style="text-align: center" width="20%">电话</td>
        <td style="text-align: center">${data.get(0).get("dhX20101400002")}</td>
        <td style="text-align: center" width="20%">地址</td>
        <td style="text-align: center">${data.get(0).get("dzX20101400002")}</td>
    </tr>

    <tr>
        <td style="text-align: center" width="20%">部门主任</td>
        <td style="text-align: center" colspan="3">${data.get(1).get("recordName")}</td>
    </tr>
    <tr>
        <td style="text-align: center" width="20%">部门主管领导</td>
        <td style="text-align: center" colspan="3">${data.get(2).get("recordName")}</td>
    </tr>
</table>
<table style="width: 756px;height: 20px; overflow-y:scroll">
    <tr>
        <td style="text-align: left" width="30%">涂抹及修改无效</td>
        <td style="text-align: left" width="70%">领用发票后,如发生票据丢失,收费不到位等情况,本人承担相应的责任</td>
    </tr>
</table>
<!--endprint1-->
</div>
