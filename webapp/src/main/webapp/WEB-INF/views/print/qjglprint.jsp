...
<table border="1" cellspacing="0" style="width: 756px;height: 750px">
    <tr style="height: 10%">
        ...
        <td style="width: 25%; text-align: center">
            <c:if test="${data.get(0).size()>0 && data.get(0) != null}">
            <c:forEach items="${data.get(0)}" var="da" varStatus="status">
                ${da.get("sqr1908030002")}
            </c:forEach>
        </c:if></td>
    </tr>
    <tr style="height: 10%">
        <td style="width: 25%;text-align: center">参加工作时间</td>
        <td style="width: 25%;text-align: center">
            <c:if test="${data.get(0).size()>0 && data.get(0) != null}">
                <c:forEach items="${data.get(0)}" var="da" varStatus="status">
                    ${da.get("cjgzs19080302")}
                </c:forEach>
            </c:if>
        </td>
        <td style="width: 25%; text-align: center">请假类别</td>
        <td style="width: 25%; text-align: center">
            <c:if test="${data.get(0).size()>0 && data.get(0) != null}">
                <c:forEach items="${data.get(0)}" var="da" varStatus="status">
                    ${da.get("qjlb190803002")}
                </c:forEach>
            </c:if></td>
    </tr>
    <tr style="height: 10%">
        <td style="width: 25%;text-align: center">事由</td>
        <td style="width: 75%;text-align: center" colspan="4">
            <c:if test="${data.get(0).size()>0 && data.get(0) != null}">
                <c:forEach items="${data.get(0)}" var="da" varStatus="status">
                    ${da.get("qjsy190803002")}
                </c:forEach>
            </c:if>
        </td>
    </tr>
    <tr style="height: 10%">
        <td style="width: 25%;text-align: center">起止时间</td>
        <td style="width: 75%;text-align: center" colspan="4">
            <c:if test="${data.get(0).size()>0 && data.get(0) != null}">
                <c:forEach items="${data.get(0)}" var="da" varStatus="status">
                    ${da.get("qjkss19080302")}起${da.get("qjjss19080302")}止,共计${da.get("qjts190803002")}天
                </c:forEach>
            </c:if>
        </td>
    </tr>
    ...
    <p>注： 1、请假手续需至少提前一天办理；</p>
    <p>    2、部长外出请指定部门临时负责人；</p>
    <p>    3、年休假、探亲假需到人力资源部审核、报备；</p>
    <p>    4、此表可复印；</p>
<!--endprint1-->
</div>
</body>
