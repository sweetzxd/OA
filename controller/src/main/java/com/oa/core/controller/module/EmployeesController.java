...
public class EmployeesController {
    ...
    @ResponseBody
    public String getEmployeesAll(HttpServletRequest request, String inputval, String option) throws UnsupportedEncodingException {
        if(inputval !=null && inputval != ""){
            inputval = new String(inputval.getBytes("iso-8859-1"), "UTF-8");
        }
        Employees employees = new Employees();
        if (inputval != null && inputval != "") {
            if ("staffId".equals(option)) {
    ...
    public String insertSave(HttpServletRequest request, Employees employees) {
        ...
        try {
            ...
            employees.setRecordTime(now);
            employees.setModifyTime(now);
            ...
            computer.setUserMenu(au.resetMenu(userName));
            computer.setRecordTime(now);
            computer.setModifyTime(now);
            userComputerService.insert(computer);

            Map<String,Object> afxtx = new HashMap();
            afxtx.put("recorderNO",pk.getNextId("afxtx19062101", "recorderNO"));
            afxtx.put("gh19062100001",0);
            afxtx.put("xm19062100001",userName);
            afxtx.put("bm19062100001",employees.getDepartment());
            afxtx.put("yhlx190621001",0);
            afxtx.put("reserveField",employees.getStaffName());
            afxtx.put("recordName","admin");
            afxtx.put("recordTime",now);
            afxtx.put("modifyName","admin");
            afxtx.put("modifyTime",now);
            tableService.insertSqlMap(MySqlUtil.getInsertSql("afxtx19062101",afxtx));
            return "1";
        } catch (Exception e) {
            return "0";
        }
